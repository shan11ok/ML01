'''
集成方法
bagging：自举汇聚法 booststrap aggregating：从原始数据集选择S次后得到S个新数据集的技术，
分类器串行训练，每个新分类器都根据已训练出的分类器的性能来进行训练
如：随机森林random forest
boosting：集中关注被已有分类器错分的那些数据来获得新的分类器，错误分类器权重增加，使用弱分类器和多个实例来构建一个强分类器
如：Adaboosting （自适应boosting）adaptive bossting
'''
